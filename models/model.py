...
def _load_mocov2_checkpoint(model, checkpoint_path):
    ...
    for raw_key, value in source_state.items():
        key = raw_key
        for prefix in ('module.encoder_q.', 'encoder_q.', 'module.', 'model.'):
    ...
    print(f'Loaded MoCo pretrained parameters from {checkpoint_path}: {len(loaded_state)} tensors')
...
class ResNetSeries(nn.Module):
    def __init__(self, pretrained, moco_checkpoint='moco_r50_v2-e3b0c442.pth'):
        ...
        if pretrained == 'supervised':
            ...
            _load_mocov2_checkpoint(model, moco_checkpoint)
...
class Network(nn.Module):
    def __init__(self, pretrained='mocov2', cin=None, moco_checkpoint='moco_r50_v2-e3b0c442.pth'):
        super(Network, self).__init__()

        self.backbone = ResNetSeries(pretrained=pretrained, moco_checkpoint=moco_checkpoint)
...
def get_model(pretrained, cin=2048+1024, moco_checkpoint='./moco_v2_200ep_pretrain.pth.tar'):
    return Network(pretrained=pretrained, cin=cin, moco_checkpoint=moco_checkpoint)
