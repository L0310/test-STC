...
def tran_param(config):
    ...
    step = 64
    if patch_size > 1:
        step = (64 // patch_size) * patch_size
        if step < patch_size:
            step = patch_size
    scale = base_size + int(np.random.choice([-2, -1, 1, 2], 1) * step)
    if patch_size > 1:
        scale = int(round(scale / patch_size) * patch_size)
        scale = max(scale, patch_size)
    param['scale'] = scale
    return param
...
def transform(image, is_mask=True, config=None):
    ...
    if is_mask:
        temp = F.interpolate(image, size=(input_size, input_size), mode='nearest')
    else:
        temp = F.interpolate(image, size=(input_size, input_size), mode='bilinear', align_corners=True)
...
def de_transform(image, is_mask=True, config=None):
    ...
    if is_mask:
        temp = F.interpolate(image, size=(input_size, input_size), mode='nearest')
        mask = F.interpolate(mask, size=(input_size, input_size), mode='nearest')
    else:
        temp = F.interpolate(image, size=(input_size, input_size), mode='bilinear', align_corners=True)
