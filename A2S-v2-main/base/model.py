...
from .encoder.vit import vit_base
...
def Network(net_name, config):
    if config['backbone'] == 'vgg':
        ...
        encoder = resnet(pretrained=config['pretrain'], layer_strides=config.get('encoder_strides'))
        fl = [64, 256, 512, 1024, 2048]
    elif config['backbone'] == 'resnet101':
        encoder = resnet101(pretrained=True, layer_strides=config.get('encoder_strides'))
        fl = [64, 256, 512, 1024, 2048]
        ...
        encoder = vit_base(pretrained=config['pretrain'])
        fl = encoder.feat_channels
    else:
