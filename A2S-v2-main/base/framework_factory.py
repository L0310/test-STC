...
def load_framework(net_name):
    ...
    if optim == 'SGD':
        if 'params' in config.keys():
            module_lr = []
            param_owner = orig_model
            for p in config['params']:
                if isinstance(p, dict):
                    group = dict(p)
                    module_name = group.pop('module')
                    group['params'] = getattr(param_owner, module_name).parameters()
                    module_lr.append(group)
                else:
                    module_lr.append({'params' : getattr(param_owner, p[0]).parameters(), 'lr' : p[1]})
            if any(isinstance(p, dict) for p in config['params']):
                optimizer = SGD(params=module_lr)
            else:
