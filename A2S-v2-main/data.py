...
def get_color_list(name, config, phase):
    ...
    if config['stage'] > 1 and phase == 'train':
        gt_root = config.get('pseudo_root') or './pseudo/c/{}'.format(name)
...
def get_train_image_list(names, config):
    ...
    if 'c' in names:
        try:
            ...
            image_list += ccc
            effective_modalities.append('c')
        except FileNotFoundError as exc:
            print('Skip training modality c because {}.'.format(exc))
    if 'd' in names:
        try:
            ddd = get_rgbd_list('RGBD-TR', config, phase)
            image_list += ddd
            effective_modalities.append('d')
        except FileNotFoundError as exc:
            print('Skip training modality d because {}.'.format(exc))
    if 'o' in names:
        try:
            ooo = get_frame_list('VSOD-TR', config, phase)
            image_list += ooo
            effective_modalities.append('o')
        except FileNotFoundError as exc:
            print('Skip training modality o because {}.'.format(exc))
    if 't' in names:
        try:
            ttt = get_rgbt_list('VT5000-TR', config, phase)
            image_list += ttt
            effective_modalities.append('t')
        except FileNotFoundError as exc:
            print('Skip training modality t because {}.'.format(exc))

    effective_trset = ''.join(effective_modalities)
    if effective_trset != names:
        print('Use available training modalities "{}" instead of requested "{}".'.format(effective_trset, names))
        config['trset'] = effective_trset
    if len(image_list) == 0:
        raise FileNotFoundError('No training data found under {} for trset={}.'.format(config['data_path'], names))
...
def get_test_list(modes='cr', config=None):
    ...
    for mode in modes:
        ...
        if subset == 'e':
            if modal == 'c':
                test_list = ['DUTS-TE', 'ECSSD',  'HKU-IS', 'PASCAL-S','DUT-OMRON'] # , 'SOD'
                # test_list = ['DUTS-TR']
            ...
            for test_set in test_list:
                ...
                try:
                    ...
                    print('Skip test set {} because {}.'.format(set_name, exc))
        else:
            if modal == 'c':
    ...
    if len(test_dataset) == 0:
        print('Warning: no available test datasets were found for vals={}.'.format(modes))
...
class Train_Dataset(data.Dataset):
    def __init__(self, config):
        self.config = config
        self.image_list = get_train_image_list(config['trset'], config)
    ...
    def __getitem__(self, index):
        ...
        elif self.config.get('allow_missing_gt', False):
        ...
        flip = False if self.config.get('disable_flip', False) else random.random() > 0.5
        if flip:
            image = image[:, ::-1].copy()
            gt = gt[:, ::-1].copy()
