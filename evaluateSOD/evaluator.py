...
class Eval_thread():
    ...
    def run(self):
        ...
        mae = self.Eval_mae()
        f,a_f = self.Eval_fmeasure()
        max_f = f.max().item()
        mean_f = f.mean().item()
        adp_f = a_f.item()
        e = self.Eval_Emeasure()
        max_e = e.max().item()
        mean_e = e.mean().item()
        s = self.Eval_Smeasure()
        self.LOG('{} dataset with {} method get {:.4f} mae, {:.4f} max-fmeasure, {:.4f} adp-fmeasure,{:.4f} max-Emeasure, {:.4f} S-measure..\n'.format(self.dataset, self.method, mae, max_f,adp_f, max_e, s))
        msg = '[cost:{:.4f}s]{} dataset with {} method get {:.4f} mae, {:.4f} max-fmeasure, {:.4f} adp-fmeasure,{:.4f} max-Emeasure,{:.4f} S-measure..'.format(time.time()-start_time, self.dataset, self.method, mae, max_f, mean_f,max_e, s)
        msg_mae = mae
        return msg_mae, msg
    def Eval_mae(self):
