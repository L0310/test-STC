...
def infer_ccam(model, ori_image, flag, hith=0.5, loth=0.2):
    ...
    with torch.no_grad():
        def get_cam(image, scale):
            ...
            cams = _flip_raw_cam(_forward_cam(model, image), flag)
...
scales = [1, 0.5, 1.5]
# 默认阈值，允许通过环境变量覆盖
hith = float(os.environ.get('HITH', 0.55))
loth = float(os.environ.get('LOTH', 0.15))
assert hith > 0. and loth > 0.
high, low = hith, loth

def _forward_cam(model, inputs):
    try:
        outputs = model(inputs, cam_only=True)
    except TypeError as exc:
        if "cam_only" not in str(exc):
            raise
...
def _flip_raw_cam(cam, flag):
    if flag:
        return -cam
    return cam

def light_cam(model, inputs, flag, return_cam=False):
    cam_list = []
    b, c, h, w = inputs.shape
    with torch.no_grad():
        inputs_cat = torch.cat([inputs, inputs.flip(-1)], dim=0)

        _cam = _flip_raw_cam(_forward_cam(model, inputs_cat), flag)
...
def _refine_cams(ref_mod, images, cams, valid_key, orig_size):
    ...
    valid_key = valid_key.to(device=refined_cams.device, dtype=torch.long)
    refined_label = refined_cams.argmax(dim=1).long()
...
def refine_cam(ref_mod, images, cams, down_scale=2):
    ...
    valid_key = torch.tensor([0, 1], device=cams.device, dtype=torch.long)
